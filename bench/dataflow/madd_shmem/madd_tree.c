 ...
 unsigned int cols, int *r, unsigned int *num_sum_matrices)
{
	...
	for (i = 0, j = 0; i < num_input_matrices; i += 2, j++) {
		...
		/* perform addition */
		if (madd_gpu(a_key, b_key, &output_matrix_keys[j], rows,
		 cols)) {
 ...
 *sum_matrix_keys, unsigned int *sum_matrix, unsigned int num_input_matrices,
 int sum_matrix_allocated)
{
	unsigned int i;

	free(sum_matrix_keys);
...
int main(int argc, char **argv)
{
	...
	sum_matrix_keys = run_binary_tree(&device_info, input_matrix_keys,
	 num_input_matrices, rows, cols, &r, &num_sum_matrices);

	/* make sure there is only one sum matrix */
	if (r || num_sum_matrices != 1) {
		...
		free_matrices(input_matrices, input_matrix_keys,
		 sum_matrix_keys, sum_matrix, num_input_matrices, 0);

		return -1;
	}

	/*
	 * Allocate memory for sum matrix and copy it from shared memory to
	 * host memory
	 */
	...
	if (shmem_device_copy(sum_matrix_keys[0], matrix_alloc_size, sum_matrix,
	 0)) {
		free_matrices(input_matrices, input_matrix_keys,
		 sum_matrix_keys, sum_matrix, num_input_matrices, 1);
	...
	free_matrices(input_matrices, input_matrix_keys, sum_matrix_keys,
	 sum_matrix, num_input_matrices, 1);
