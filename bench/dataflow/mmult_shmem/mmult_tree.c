 ...
 unsigned int cols, int *r, unsigned int *num_product_matrices)
{
	...
	for (i = 0, j = 0; i < num_input_matrices; i += 2, j++) {
		...
		/* perform addition */
		if (mmult_gpu(a_key, b_key, &output_matrix_keys[j], rows,
		 cols)) {
 ...
 *product_matrix_keys, unsigned int *product_matrix, unsigned int num_input_matrices,
 int product_matrix_allocated)
{
	unsigned int i;

	free(product_matrix_keys);
...
int main(int argc, char **argv)
{
	...
	product_matrix_keys = run_binary_tree(&device_info, input_matrix_keys,
	 num_input_matrices, rows, cols, &r, &num_product_matrices);

	/* make sure there is only one product matrix */
	if (r || num_product_matrices != 1) {
		...
		free_matrices(input_matrices, input_matrix_keys,
		 product_matrix_keys, product_matrix, num_input_matrices, 0);

		return -1;
	}

	/*
	 * Allocate memory for product matrix and copy it from shared memory to
	 * host memory
	 */
	if ((product_matrix = (unsigned int *)malloc(matrix_alloc_size)) == NULL) {
		free_matrices(input_matrices, input_matrix_keys,
		 product_matrix_keys, product_matrix, num_input_matrices, 0);
	...
	if (shmem_device_copy(product_matrix_keys[0], matrix_alloc_size, product_matrix,
	 0)) {
		free_matrices(input_matrices, input_matrix_keys,
		 product_matrix_keys, product_matrix, num_input_matrices, 1);
	...
	for (i = 0; i < rows * cols && !has_failed; i++) {
		if (product_matrix[i] != EXPECTED_SUM) {
			printf("product_matrix[%u] was %u, but expected %u\n",
			 i, product_matrix[i], EXPECTED_SUM);	
	...
	*/
	...
	free_matrices(input_matrices, input_matrix_keys, product_matrix_keys,
	 product_matrix, num_input_matrices, 1);
