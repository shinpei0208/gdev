 ...
 unsigned int cols, int *r, unsigned int *num_sum_matrices)
{
	...
	for (i = 0, j = 0; i < num_input_matrices; i += 2, j++) {
		...
		/* perform addition */
		if (madd_gpu(device_info, a, b, output_matrices[j], rows,
		 cols)) {
			for (k = 0; k < j; k++)
	...
	*num_sum_matrices = num_output_matrices;
	...
	final_output_matrices = run_binary_tree(device_info, output_matrices,
	 num_output_matrices, rows, cols, r, num_sum_matrices);
...
int main(int argc, char **argv)
{
	...
	*/
	...
	sum_matrix = run_binary_tree(&device_info, input_matrices,
	...
	if (r || num_sum_matrices != 1) {
		...
		free_matrices(input_matrices, sum_matrix, num_input_matrices,
		 num_sum_matrices, 0);
	...
	*/
	...
	for (i = 0; i < rows * cols && !has_failed; i++) {
		if (sum_matrix[0][i] != EXPECTED_SUM) {
			printf("sum_matrix[%u] was %u, but expected %u\n",
			 i, sum_matrix[0][i], EXPECTED_SUM);	
	...
	free_matrices(input_matrices, sum_matrix, num_input_matrices, 1, 1);
