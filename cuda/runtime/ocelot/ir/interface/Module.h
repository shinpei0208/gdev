...
namespace ir {
	...
	public:
		...
		*/
		...
		/*!	Unloads module and loads cubin via a destructive copy */
		bool lazyLoadCubin(std::string& binary,
			const std::string& path = "::unknown path::");
		
		/*!	Unloads module and loads cubin, this pointer must be valid 
			until the module is loaded */
		bool lazyLoadCubin(const char* source,
			const std::string& path = "::unknown path::");
		
		const char* getPTX() const;
		const char* getCubin() const;
	...
	private:
		...
		/*! \brief This is a copy of the original cubin for lazy loading */
		std::string _cubin;
		/*! \brief This is a pointer to the original cubin
			for lazy loading */
		const char* _cubinPointer;
