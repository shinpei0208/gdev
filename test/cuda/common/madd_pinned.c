...
int cuda_test_madd_pinned(unsigned int n, char *path)
{
	int i, j, idx;
	CUresult res;
	CUdevice dev;
...
*/
	...
	/* a[] */
	res = cuMemAllocHost((void **)&a_buf, n*n * sizeof(unsigned int));
	if (res != CUDA_SUCCESS) {
		printf("cuMemAllocHost (a) failed\n");
		return -1;
	}
	/* b[] */
	res = cuMemAllocHost((void **)&b_buf, n*n * sizeof(unsigned int));
	if (res != CUDA_SUCCESS) {
		printf("cuMemAllocHost (b) failed\n");
		return -1;
	}
	/* c[] */
	res = cuMemAllocHost((void **)&c_buf, n*n * sizeof(unsigned int));
	if (res != CUDA_SUCCESS) {
		printf("cuMemAllocHost (c) failed\n");
		return -1;
	}
	...
	/* upload a[] and b[] */
	res = cuMemcpyHtoD(a_dev, a_buf, n*n * sizeof(unsigned int));
	if (res != CUDA_SUCCESS) {
		printf("cuMemcpyHtoD (a) failed: res = %lu\n", (unsigned long)res);
		return -1;
	}
	res = cuMemcpyHtoD(b_dev, b_buf, n*n * sizeof(unsigned int));
	if (res != CUDA_SUCCESS) {
	...
	/* download c[] */
	res = cuMemcpyDtoH(c_buf, c_dev, n*n * sizeof(unsigned int));
	if (res != CUDA_SUCCESS) {
